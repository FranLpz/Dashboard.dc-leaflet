...
dc.leafletChart = function (_chart) {
    ...
    _chart._doRender = function () {
        var me = this;//console.log(_map)
        if (_map != undefined  && siono) { 
            //_map.remove();
            _chart._doRedraw()
            return
            //TODO clearLayers(); obtener el markerCluster Layer map.getLayer("NombreCapa")
            //return;
        }; 
        if (_map != undefined && !siono) {
            //console.log('redrow')
            _map.remove();
            dc.filterAll('grupoChart');
            //dc.redrawAll('grupoChart');
          ...
          //TODO clearLayers(); obtener el markerCluster Layer map.getLayer("NombreCapa")
            ...
            // console.log('Voy a remover el mapa')
            // _map.remove();
            // siono = false;
            // console.log(_map)
        }
...
dc.leafletMarkerChart = function (parent, chartGroup) {
    ...
    var _marker = function (d, map) {
        /**TODO FIXME UBICAME UNDEFINED IS NOT A FUNCTION AQUI TOY JAJA AQUI TOY*/
        if(d.length < 0 ){
            return;
        }
        var lonlatArray = d.key[0].split(",");
        var latlng = L.latLng(+lonlatArray[0], +lonlatArray[1]);
        
        var marker = new L.Marker(latlng, {
            title: _chart.renderTitle() ? _chart.title()(d) : '',
            alt: _chart.renderTitle() ? _chart.title()(d) : '',
            icon: _icon(),
            /*icon: new  L.FontMarkers.Icon({
                icon: (lonlatArray[2]!='null')?lonlatArray[2]:'C', 
                iconColor: "white",
                prefix:"icondenuncias-"
            }),   */
    ...
    var _icon = function (d, map) {
        //image path leaflet
        //App.get("appBasePath")+"/resources/js/site/Components/bootstrap/images"
        L.Icon.Default.imagePath = "js/images";
        return new L.Icon.Default();
    };

    var _popup = function (d, marker) { 
        //[d.latitud+','+d.longitud+','+d.icono+','+d.hora+','+d.tipo]; 
        var msg = d.key[0].split(",");
        if(msg[2]=='null'){
            msg[2] = 'C';
            msg[4] = 'Llamada'; 
        };
        ...
        return '<p><span class="app-icon icondenuncias-" style="font-size: 22px;">'+msg[2]+'</span>  '+msg[4]+'</p>\n<p>'+msg[3]+'\n</p>'
    };
    ...
    _chart._doRedraw = function () {
        ...
        //     return;
        // }
        //console.log(_currentGroups.toString())
        //console.log('i _doRedraw')
